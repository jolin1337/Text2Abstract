...
void FastText::saveModel() {
  ...
  std::ofstream ofs_vec1(args_->output + ".bin.vec1", std::ofstream::binary);
  std::ofstream ofs_vec2(args_->output + ".bin.vec2", std::ofstream::binary);
  if (!ofs.is_open() || !ofs_vec1.is_open() || !ofs_vec2.is_open()) {
    std::cerr << "Model file cannot be opened for saving!" << std::endl;
    exit(EXIT_FAILURE);
  }
  args_->save(ofs);
  dict_->save(ofs);
  input_->save(ofs_vec1);
  output_->save(ofs_vec2);
  ofs.close();
  ofs_vec1.close();
  ofs_vec2.close();
}
...
void FastText::loadModel(const std::string& filename) {
  ...
  std::ifstream ifs_vec1(filename + ".vec1", std::ifstream::binary);
  std::ifstream ifs_vec2(filename + ".vec2", std::ifstream::binary);
  if (!ifs.is_open() || !ifs_vec1.is_open() || !ifs_vec2.is_open()) {
    std::cerr << "Model file cannot be opened for loading!" << std::endl;
    exit(EXIT_FAILURE);
  }
  loadModel(ifs, ifs_vec1, ifs_vec2);
  ifs.close();
  ifs_vec1.close();
  ifs_vec2.close();
}

void FastText::loadModel(std::istream& in, std::istream& in_vec1, std::istream& in_vec2) {
...
void FastText::test(std::istream& in, int32_t k) {
  ...
  std::vector<int32_t> labelsTotal;
  std::vector<std::vector<int32_t> > documentMatrix;
  ...
  while (in.peek() != EOF) {
    ...
    if (labels.size() > 0 && line.size() > 0) {
      ...
      for (auto it = modelPredictions.cbegin(); it != modelPredictions.cend(); it++) {
        long labelSize = labelsTotal.end() - labelsTotal.begin();
        auto predpos = std::find(labelsTotal.begin(), labelsTotal.end(), it->second) - labelsTotal.begin();
        if(predpos == labelSize) {
          labelsTotal.push_back(it->second);
          std::for_each(documentMatrix.begin(), documentMatrix.end(),
                      [](std::vector<int32_t> &row) {row.push_back(0);});
          documentMatrix.push_back(std::vector<int32_t>(labelSize+1, 0));
        }
        auto truepos = std::find(labelsTotal.begin(), labelsTotal.end(), labels[0]) - labelsTotal.begin();
        documentMatrix[predpos][truepos] += 1;
